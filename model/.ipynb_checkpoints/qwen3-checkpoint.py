...
class ModifiedLQwen3Attention(Qwen3Attention):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # 这里可以根据需要覆盖属性
        self.is_causal = False
...
Qwen3_ATTENTION_CLASSES = {
    "eager": ModifiedLQwen3Attention,
    "flash_attention_2": ModifiedLQwen3Attention,
    "sdpa": ModifiedLQwen3Attention,
}
...
class Qwen3BiForMNTPandSentEmbeddingsV2(Qwen3BiForMNTP):
    def __init__(self, config):
        Qwen3PreTrainedModel.__init__(self, config)
        self.model = Qwen3BiModel(config)
        self.vocab_size = config.vocab_size
...
class Qwen3BiForMNTPandSentEmbeddingsV2_w_q_token(Qwen3BiForMNTP):
    def __init__(self, config):
        Qwen3PreTrainedModel.__init__(self, config)
        self.model = Qwen3BiModel(config)
        self.vocab_size = config.vocab_size

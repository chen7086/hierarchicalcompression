...
class DataCollatorForMNTPandContrastiveLearningV2:
    def __init__(self, 
        ...
        self.special_tokens_map = {}
        for k, tok in tokenizer.special_tokens_map.items():
            if isinstance(tok, list):
                # 针对 additional_special_tokens
                for sub_tok in tok:
                    tok_id = tokenizer.convert_tokens_to_ids(sub_tok)
                    self.special_tokens_map[tok_id] = sub_tok
            else:
                tok_id = tokenizer.convert_tokens_to_ids(tok)
                self.special_tokens_map[tok_id] = tok
